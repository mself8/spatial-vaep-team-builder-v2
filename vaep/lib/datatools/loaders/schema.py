...
class GameSchema(pa.SchemaModel):
    ...
    competition_id: Series[Object] = pa.Field()
    """The unique identifier for the competition."""
    game_day: Series[int] = pa.Field(nullable=True) # pd.Int64Dtype is not directly supported by the HDF5 format, so it cannot be saved (pandas version differences)
...
class SequenceSchema(pa.SchemaModel):
    """Definition of a dataframe containing sequences of events in a game."""

    game_id: Series[int]
    """The unique identifier for the game."""
    period_id: Series[int]
    """The unique identifier for the part of the game in which the sequence took place."""
    team_id: Series[float] = pa.Field(nullable=True) # None = Ball Neutral
    """The unique identifier for the team."""

    start_time: Series[int] 
    """The time when the sequence started."""
    end_time: Series[int]
    """The time when the sequence ended."""

    event_ids: Series[Object]
    """The list of event ids in the sequence."""
