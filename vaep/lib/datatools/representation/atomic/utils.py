...
from . import config as atomicspadlconfig
...
def add_names(actions: DataFrame[AtomicSPADLSchema]) -> DataFrame[AtomicSPADLSchema]:
    ...
    return cast(
        ...
        .merge(atomicspadlconfig.actiontypes_df(), how="left")
        .merge(atomicspadlconfig.bodyparts_df(), how="left")
...
def play_left_to_right(
    ...
    ltr_actions.loc[away_idx, "x"] = atomicspadlconfig.field_length - actions[away_idx]["x"].values
    ltr_actions.loc[away_idx, "y"] = atomicspadlconfig.field_width - actions[away_idx]["y"].values
