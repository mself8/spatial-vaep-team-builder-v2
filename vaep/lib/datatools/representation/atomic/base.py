...
import datatools.representation.spadl.config as _spadl
from datatools.representation.spadl.base import _add_dribbles, _add_dribbles_after_receive
...
def convert_to_atomic(actions: DataFrame[SPADLSchema]) -> DataFrame[AtomicSPADLSchema]:
    ...
    # TODO: _add_dribbles_after_receive is also possible, but excluded for the baseline experiments
    atomic_actions = _add_dribbles(atomic_actions)  # for some reason this adds more dribbles -> what is the reason?
    #atomic_actions = _add_dribbles_after_receive(atomic_actions, selector_receive=atomic_actions["type_id"] == _atomicspadl.actiontypes.index("receival"))
...
# Insert receival, interception, out, and offside actions as pass outcomes
def _extra_from_passes(actions: pd.DataFrame) -> pd.DataFrame:
    ...
    extra["original_event_id"] = None#prev.original_event_id
...
# Insert yellow/red card actions as foul outcomes
